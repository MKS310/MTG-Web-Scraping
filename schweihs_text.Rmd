---
title: "Topic Model of Current Magic: The Gathering Metagame (Commander Format)"
author: "Maggie Schweihs"
date: "Spring 2019"
output: pdf_document
bibliography: ["schweihs_text.bib"]
...
knitr::write_bib(c("knitr", "stringr", "rjson", "plyr", 
                   "dplyr", "mallet", "wordcloud"), "", width = 60)
library(rjson)
library(plyr)
library(dplyr)
library(ggplot2)
...
```{r load JSON file, message=FALSE, warning=FALSE, include=FALSE}
decks<- fromJSON(file = "decks.json")
#Create a succint data frame
...
COMMANDER = 1 #The commander is always at the first index

for( i in 1:numDecks ){
  deck_df$color[i] <- paste(decks[[i]]$Cards[[COMMANDER]]$colors, collapse=', ' ) #Get the commander's colors. These are also the deck's colors
  #Check if no color, then colorles
  if(deck_df$color[i] == ""){
    deck_df$color[i] = "Colorless"
  }
  #Get deck name and the number of decks 
  deck_df$name[i] <- decks[[i]]$DeckName
  deck_df$weight[i] <- as.numeric(decks[[i]]$NumberOfDecks)
  
  #Get info for each card in the deck
  ...
  for(j in 1:numCards){
    ...
    deck_text = append(deck_text,card_text)
      ...
      deck_text[j] = " " #Some basic land cards have no text
    }
  }
  deck_df$card_text[i] =  paste(deck_text, collapse = "")
}
remove(decks)
deck_df$color = factor(deck_df$color) #color as a factor for ggplot
```

\subsection{Initial Data Exploration}

I grouped decks according to color-type \footnote{In the Commander format, there is one card that is the "Commander" of the deck. This card has special play rules and properties. The colors that appear on the Commander dictate which colored cards are allowed to be included in the deck. For example, in a deck with a Commander of red/blue type, a player cannot have white, green, or black cards; only red and blue cards are allowed. }.
...
\pagebreak
\section{Visualizations and Findings}
